---
description: Set up an isolated worktree to work on a Linear issue (worktree + install + session summary) [isi]
...
creating the worktree off the baseline, writing per-app prep files, and running each
app's install/setup — is done by **`issue setup`** (the devkit
...
Read live URLs back from `devrun status`.
...
- the branch name
- how to start servers: `devrun up`, then `devrun status` to see them

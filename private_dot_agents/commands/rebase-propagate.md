...
`~/.claude/scripts/rebase-propagate.sh` has **already run**. It fetched, discovered
...
!`bash ~/.claude/scripts/rebase-propagate.sh "$ARGUMENTS" 2>&1 || true`
...
3. `git add <files>` then `git rebase --continue`, repeating for further conflicts on
   that branch.
4. Re-run `bash ~/.claude/scripts/rebase-propagate.sh`. It resumes the same run from
...
`bash ~/.claude/scripts/rebase-propagate.sh --dupes` once — it is read-only. Never

---
name: validate-tdd-vt
description: Validate that the TDD tests added for this fix/feature actually exercise the bug end-to-end and prove the fix — not ceremony unit tests that pass regardless
...
The rubric is the **Testing** section of the user's `~/.claude/CLAUDE.md`. The single
...
`$ARGUMENTS` = optional base ref/branch to diff against (e.g. `main`, `origin/main`, a
...
Before step 1, build the task list with the **built-in `TaskCreate` tool** — one call per
task, passing `subject` (brief title) and `description` (what needs doing) as top-level
strings. It has no `tasks`/`todos` array parameter, so don't batch them. Seed it with one
task per numbered step below; once step 3 enumerates the tests, add one task per
test-under-audit. Drive status with `TaskUpdate`: exactly one `in_progress` at a time,
flipped to `completed` only once that step's evidence is in hand.

Don't hand this off to the `checklist` skill and don't write the list out as markdown in
your reply — a rendered task list is how the user watches progress, and a chat-formatted
one isn't it. Stop and ask the user if anything is ambiguous — never leave the tree in a
reverted state.
...
Resolve `BASE` first: if `$ARGUMENTS` is non-empty, use it verbatim; otherwise auto-detect.
Then diff:

```bash
# BASE = $ARGUMENTS if given, else the merge-base with main/master

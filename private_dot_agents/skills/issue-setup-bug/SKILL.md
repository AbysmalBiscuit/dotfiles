---
name: issue-setup-bug-isb
description: Set up an issue worktree via /issue-setup for a bug, fanning Vercel/Sentry/PostHog recon out to subagents, then tracing the error path with /graphify
...
`$ARGUMENTS` = whatever identifies the bug:
...
If `$ARGUMENTS` is empty, ask the user for one of the above before doing anything.

## Steps

Before step 0, create the task list with **`TaskCreate`** (one call per task, `subject` +
`description` as top-level strings — it has no array parameter). Seed it with one task per
numbered step, plus one per subagent once step 2 dispatches them. Drive status with
`TaskUpdate`: exactly one `in_progress` at a time. Don't write the checklist into your
reply as markdown — the rendered list is how the user follows along.
...
Dispatch all three with the **`Agent`** tool in a **single message** so they run
concurrently. Give each one the full error signature from step 0. Each is **read-only** and
...
**Subagent C — PostHog (who hit it).** Use `mcp__plugin_posthog_posthog__exec` for HogQL over
the events table, and the error-tracking / session-replay surfaces.
...
When the gate passes, dispatch **one `Agent`** to answer *when* and *what* changed. It is

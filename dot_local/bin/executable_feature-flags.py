...
Counts are SDK-deduplicated evaluations, not HTTP requests, and carry no
environment, so read them as a ranking and a floor. `off` also absorbs
`flag_missing`, which is what a disabled flag returns, so its "off" count is
every evaluation and says nothing about rollout. `sweep` holds the evaluations
where one identity walked >= `--sweep-min` distinct flags in five minutes, and
`ids` counts the identities left after those come out.
"""
...
# Distinct flags one identity may evaluate in five minutes before it reads as a
# harness enumerating the list. `--sweep-min 0` counts every evaluation as usage.
SWEEP_MIN = 15
...
def sweep_flag(window: str, sweep_min: int) -> str:
    """Predicate marking an event as part of a flag-list enumeration.

    The inner scan is never narrowed to one flag: a sweep is only visible across
    the whole stream, so a per-flag query still has to look at all of it.
    """
    if sweep_min <= 0:
        return "false"
    return (
        "concat(distinct_id, '|', toString(toStartOfFiveMinutes(timestamp))) IN ("
        "SELECT concat(distinct_id, '|', toString(toStartOfFiveMinutes(timestamp))) "
        f"FROM events WHERE event = '$feature_flag_called' AND {window} "
        "GROUP BY distinct_id, toStartOfFiveMinutes(timestamp) "
        f"HAVING uniq(properties.$feature_flag) >= {sweep_min})"
    )


CALL_FIELDS = ("n_off", "n_on", "n_err", "n_ids", "n_sweep")
...
def fetch_calls(days: int, key: str | None = None) -> dict[str, dict[str, int]]:
    ...
    sweep_min = SWEEP_MIN
    tagged = (
        "SELECT properties.$feature_flag AS flag, distinct_id, "
        "JSONExtractRaw(properties, '$feature_flag_response') AS resp, "
        "JSONExtractRaw(properties, '$feature_flag_error') AS err, "
        f"{sweep_flag(window, sweep_min)} AS sweep "
        f"FROM events WHERE event = '$feature_flag_called' AND {window}"
    )
    outer = "" if key is None else f" WHERE flag = {quote(key)}"
    rows = run_sql(
        "SELECT flag, "
        "countIf(NOT sweep AND (resp = 'false' OR err = '\"flag_missing\"')) AS n_off, "
        "countIf(NOT sweep AND resp NOT IN ('', 'false', 'null')) AS n_on, "
        "countIf(NOT sweep AND err NOT IN ('', '\"flag_missing\"')) AS n_err, "
        "uniqIf(distinct_id, NOT sweep) AS n_ids, "
        "countIf(sweep) AS n_sweep "
        f"FROM ({tagged}){outer} GROUP BY flag"
    )
    return {r["flag"]: {k: int(r[k] or 0) for k in CALL_FIELDS} for r in rows}
...
def recent_evaluations(keys: list[str], hours: int) -> dict[str, tuple[int, str]]:
    """Count and newest timestamp per flag, ignoring sweeps, for the archive guard.

    The timestamp is the actionable half: a count alone cannot say whether the
    traffic predates the retirement deploy or is still arriving after it.
    """
    quoted = ", ".join(quote(k) for k in keys)
    window = f"timestamp > now() - INTERVAL {hours} HOUR"
    with spinner(f"checking the last {hours}h of evaluations"):
        rows = run_sql(
            "SELECT flag, count() AS n, toString(max(ts)) AS last FROM ("
            "SELECT properties.$feature_flag AS flag, timestamp AS ts, "
            f"{sweep_flag(window, SWEEP_MIN)} AS sweep "
            f"FROM events WHERE event = '$feature_flag_called' AND {window}"
            f") WHERE NOT sweep AND flag IN ({quoted}) GROUP BY flag"
        )
    return {r["flag"]: (int(r["n"] or 0), r["last"] or "") for r in rows}
...
def render(flags: list[dict], days: int, color: bool) -> None:
    ...
    notes = [f"evaluations over the last {days} days, deduplicated by the SDK"]
    ...
    if any(f["calls"]["n_sweep"] for f in flags):
        notes.append("sweep = flag-list enumeration, held out of off/on")
    print("\n" + "; ".join(notes))
            ...
            [
                {
                    ...
                    "active": f["active"],
                    "archived": f["archived"],
                    "evals_off": f["calls"]["n_off"],
                    "evals_on": f["calls"]["n_on"],
                    "evals_error": f["calls"]["n_err"],
                    "identities": f["calls"]["n_ids"],
                    "evals_sweep": f["calls"]["n_sweep"],
                    "tags": f.get("tags", []),
                }
                for f in flags
            ],
...
NUMERIC = {
    ...
    "ids": lambda f: f["calls"]["n_ids"],
    "sweep": lambda f: f["calls"]["n_sweep"],
...
def vocabulary() -> str:
    ...
    return (
        ...
        "  off, on, reqs, ids exclude sweeps; sweep counts what they excluded"
    )
...
def filter_help() -> str:
    ...
    return (
        ...
        "  -f 'ids.lt.2,sweep.gt.0'                only a harness has touched it\n"
        "\n"
        f"{vocabulary()}"
    )
...
EMPTY_CALLS = dict.fromkeys(CALL_FIELDS, 0)
...
def cmd_list(args) -> None:
    ...
    keys = {
        ...
        "rollout": rollout_of,
        "evals": lambda f: f["calls"]["n_off"] + f["calls"]["n_on"],
...
def cmd_archive(args) -> None:
    ...
    if disabling and not args.force:
        serving = {
            key: seen
            for key, seen in recent_evaluations(disabling, args.since).items()
            if seen[0]
        }
        if serving:
            listing = "\n  ".join(
                f"{k}: {n:,}, last at {last}" for k, (n, last) in serving.items()
            )
            raise SystemExit(
                "Archiving disables the flag, so a caller that evaluated it in "
                f"the last {args.since}h would fall through to its off branch:\n  "
                f"{listing}\n"
                "Compare those timestamps against when the retirement deployed. "
                "Traffic from before it is stale; traffic after it is a live "
                "caller on an old build. Then wait, or pass --force."
            )
    if args.dry_run:
        print("dry run, nothing was written.")
        return
...
def build_parser() -> argparse.ArgumentParser:
    ...
    common.add_argument(
        "--days", type=int, default=30, help="evaluation-count window (default 30)"
    )
    common.add_argument(
        "--sweep-min",
        type=int,
        default=SWEEP_MIN,
        metavar="N",
        help=f"distinct flags per identity per 5 min that read as a flag-list "
        f"sweep, held out of off/on (default {SWEEP_MIN}; 0 counts everything)",
    )
    ...
    listing.add_argument(
        "--sort",
        choices=["changed", "created", "evals", "ids", "name", "rollout", "requests"],
        default="changed",
    )
...
def main() -> int:
    ...
    global SWEEP_MIN
    SWEEP_MIN = getattr(args, "sweep_min", SWEEP_MIN)

...
---Converts a passed color to a pastel version.
---@param hex_str string hexadecimal value of a color
---@param hex_mixin string? hexadecimal value of the mixin color. Defaults to white #ffffff
M.pastelify = function(hex_str, hex_mixin)
  hex_mixin = hex_mixin or "#ffffff"

  local hsl = hsluv.hex_to_hsluv(hex_str)
  hsl[2] = hsl[2] * 1.1
  local rgb = hsluv.hsluv_to_rgb(hsl)
  for i, v in pairs(rgb) do
    rgb[i] = math.min(math.floor(v * 255), 255)
  end

  local saturated = rgb2Hex(rgb)
  local saturated_rgb = hex_to_rgb(saturated)
  local rgb_mixin = hex_to_rgb(hex_mixin)
  return rgb2Hex({
    math.min(math.floor((saturated_rgb[1] + rgb_mixin[1]) / 2), 255),
    math.min(math.floor((saturated_rgb[2] + rgb_mixin[2]) / 2), 255),
    math.min(math.floor((saturated_rgb[3] + rgb_mixin[3]) / 2), 255),
  })
end

...
local function toggle_tmpl_lines(line1, line2)
  ...
  for _, l in ipairs(lines) do
    table.insert(all_ranges, find_tmpl_ranges(l))
  end

  local should_comment = false
  for i = 1, #all_ranges do
    local ranges = all_ranges[i]
    for _, r in ipairs(ranges) do
...
local function toggle_lines_normal(line1, line2)
  ...
  local should_comment = false
  for _, l in ipairs(lines) do
    if l:match("%S") and not is_line_commented(l, cs_left) and not is_line_commented(l, "^%s*{{%-?%s*/%*") then
      should_comment = true
      break
    end
  end
...
---@param insert_function_name string Name of function that should be wrapped in dot repeat.
---@param repeat_command string? Command to run for dot repeat.
local function dot_repeat(insert_function_name, repeat_command)
  repeat_command = (repeat_command ~= nil and repeat_command ~= "") and repeat_command or "g@"
  return function()
    vim.go.operatorfunc = "v:lua.require'gotmpl_comment'." .. insert_function_name
    return vim.api.nvim_input(repeat_command)
  end
end
...
function M.setup(opts)
  opts = opts or { buffer = true, silent = true }

  vim.keymap.set("n", "gcc", dot_repeat("op_gcc", "g@_"), opts)
  vim.keymap.set("n", "gc", dot_repeat("op_motion"), vim.tbl_deep_extend("force", opts, { expr = true }))
  vim.keymap.set("v", "gc", dot_repeat("op_visual"), opts)
end

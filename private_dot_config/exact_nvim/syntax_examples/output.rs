 ...
 * This has a `inline code`
 */
fn will_error(arg1: Vec<u64>) -> Vec<i64> {
  arg1.iter().map(|num| num as i64)
}

impl OutputType {
    fn testing<'a>(text: &'a str) -> &'a str {
        let var = vec![0, 1, 2];
        var.iter().collect();
        todo!()
    }
    ...
    pub fn handle(&mut self) -> Result<&mut dyn Write> {
        Ok(match *self {
            ...
            OutputType::Stdout(ref mut handle: &mut Stdout: &mut Stdout: &mut Stdout) => handle,
        })
    }
}impl OutputType

---
name: codex-build
description: Use when a written implementation plan or frozen spec exists (e.g. from superpowers:writing-plans, or after /codex-review or /grill-me-codex hardens it) and you want a second model to IMPLEMENT it — an alternative to superpowers:executing-plans and superpowers:subagent-driven-development when the work is delegatable. OpenAI Codex builds with write access while Claude stays reviewer: reads the diff like a contributor PR, runs the proof test, human approves before any commit. Also for standalone delegation: refactors, mechanical migrations, bug fixes with a known repro, test/coverage writing. Triggers on "/codex-build", "have codex build/implement this", "delegate the build to codex", "use codex to write the code". Requires the codex CLI installed and authenticated. NOT for tiny edits (~<20 lines), NOT for design work (spec it first via /grill-me-codex), NOT for reviewing existing code, NOT for tasks needing Claude-session tools (MCP, secrets, browser).
---

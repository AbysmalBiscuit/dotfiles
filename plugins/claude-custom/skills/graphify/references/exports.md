...
This starts a stdio MCP server that exposes tools: `query_graph`, `get_node`, `get_neighbors`, `get_community`, `god_nodes`, `graph_stats`, `shortest_path`. Add it to Codex or another MCP-compatible orchestrator so agents can query the graph live.

When configuring an MCP client, do not use `$(...)`: set `command` to the **absolute
interpreter path** printed by `cat graphify-out/.graphify_python`:
```json
{
  "mcpServers": {
    "graphify": {

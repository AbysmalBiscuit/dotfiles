...
> **No other API keys are read.** When `GEMINI_API_KEY`/`GOOGLE_API_KEY` are unset,
> semantic extraction falls to the running Codex session. Dispatch sub-agents as written
> in Part B. A code-only corpus has no semantic work, so write the empty semantic file and
> continue to Part C. Never prompt for an Anthropic or OpenAI API key for this workflow.
...
**MANDATORY: Use Codex sub-agents here. Reading files yourself one-by-one is forbidden
because it is 5-10x slower.** This instruction explicitly authorizes delegation for this
extraction step.
...
**Step B2 - Dispatch all subagents concurrently**

Spawn one Codex sub-agent per chunk before waiting for any result. Respect the available
concurrency limit and process additional chunks in parallel waves.

Concrete example for 3 chunks:
```
[sub-agent task 1: files 1-15]
[sub-agent task 2: files 16-30]
[sub-agent task 3: files 31-45]
```
Start all three before waiting.
...
- If the file is missing, print a warning: "chunk N missing from disk — re-run that writable sub-agent task." Do not silently skip.
- If a subagent failed or returned invalid JSON, print a warning and skip that chunk - do not abort

If more than half the chunks failed or are missing, stop and tell the user to re-run the
failed writable sub-agent tasks.

Merge all chunk files into `.graphify_semantic_new.json`. If the sub-agent result exposes
token usage, write it into the chunk JSON before merging; otherwise leave the placeholder
zeros. Then run:
```bash
$(cat graphify-out/.graphify_python) -c "
import json, glob
...
When the user asks to install the post-commit auto-rebuild hook or wire graphify into a
project's `AGENTS.md`, see `references/hooks.md`.

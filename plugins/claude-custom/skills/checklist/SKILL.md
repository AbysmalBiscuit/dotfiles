...
If the work has 3+ distinct steps, create one item per step with the available Codex
plan/task-list mechanism **before doing anything else**. Then, for every step:
...
Use Codex's plan/task-list mechanism when available so progress remains updateable. If
it is unavailable, keep a concise checklist in commentary instead of pretending a
persistent task list exists.
...
| "I'll just write the list in my reply" | Prefer the available plan/task-list mechanism so status remains updateable. |

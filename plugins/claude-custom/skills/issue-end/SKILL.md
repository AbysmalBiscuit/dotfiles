---
name: issue-end
description: "Find finished issue worktrees (PR merged + Linear done), triage their artifacts, then remove worktree + ISSUE_*.md files Use when the user invokes $issue-end or asks for this workflow."
---
...
`<USER_INPUT>` = optional issue ID(s) to limit the cleanup to (e.g. `ENG-1234 DBI-986`).
...
1. Quick triage — list files with size/date, peek at heads (HTML title / first heading)
   ...
   output).
2. Ask the user directly, one concise question per worktree: options like

---
name: issue-start
description: "Cold-start a session inside an issue worktree — load the ISSUE_SUMMARY handoff and orient Use when the user invokes $issue-start or asks for this workflow."
---
...
`<USER_INPUT>` = optional issue ID (e.g. `ENG-1234`). If empty, auto-detect (step 1).
...
- If `<USER_INPUT>` set → `ISSUE_ID = <USER_INPUT>`.
...
**Linear** — fetch the issue's current assignee through the configured Linear integration.

- **No one assigned** → resolve the current user and assign them through the configured
  Linear integration.
- **Someone already assigned** →
  - If `<USER_INPUT>` (or the conversation) included explicit permission to
...
- If the summary already lists a **Sentry** issue → confirm it's assigned to the user
  in Sentry; assign it through the configured Sentry integration if not.
- If the summary lists **no Sentry** issue → do one fresh search
  using the configured Sentry integration and the error text / affected module /
  ...
  - add a Linear comment with the Sentry URL and short ID,
